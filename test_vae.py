...
def visualization_model(model, args, test_dataloader, name_info):
    ...
    with torch.no_grad():
        for i in range(100):
            ...
            data_input = torch.randn(data_input.shape).cuda() ################ This is for random latent point testing ################
            ...
            if args.output_type == "Implicit":
                ...
                visualization.plot_real_pred(gt.detach().cpu().numpy(), pred.detach().cpu().numpy(), 1, save_loc='debug/' + f"real_pred{i}.png") 
            print(save_loc + "real_pred.png")
...
def main():
    ...
    if args.train_mode == "test":
        ...
        checkpoint = torch.load(args.checkpoint +".pt")
        ...
        visualization_model(net, args, test_dataloader, 'new')
        ...
        kl_weight_scheduler = frange_cycle_cosine(0.0, 1.0e-6, args.epochs, 4) # weight scheduling
